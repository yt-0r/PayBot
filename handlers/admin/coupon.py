...
from keyboards.coupon_kb import coupon_kb
...
@router.callback_query(F.data == "adm_coupon")
async def add_address_start(callback: CallbackQuery, state: FSMContext):
    logger.info(f"{callback.from_user.username} {callback.from_user.id} press button Купоны")
    ...
    await callback.message.answer("Операции с купонами:", reply_markup=coupon_kb())
